...
public:
  ...
  unsigned registerExpensiveOp(const std::string &opTypeName) {
    ...
    LDBG("Registered expensive op type '" << opTypeName << "' with barrier ID " << barrierId);
  ...
  // Get all operations that mark the end of critical region for a specific expensive op
...
static void handleWarpSpec(ttg::WarpSpecializeOp wsOp) {
  ...
  for (Region *region : wsOp.getPartitionRegions()) {
    ...
    for (auto &loopDepth : loopDepths) {
      LDBG("loop depth " << loopDepth.first << " has " << loopDepth.second.size());
    }
    ++partitionId;
  }
  ...
  for (auto &loopDepth : partitionLoopDepths) {
    ...
    // TODO: should better check before code parition if the kernel is persistent.
    // The kernel is persistent if it has a ForOp at depth of 1.
  ...
  for (unsigned iter = 0; iter < computeRegions.size(); ++iter) {
    ...
    region->walk<WalkOrder::PreOrder>([&](Operation *op) {
      ...
      if (isa<ttng::WarpGroupDotOp>(op)) {
        ...
        if (crManager.hasPingPongBoundarySetup(op->getName().getStringRef().str())) {
          return;  // Already setup, skip
        } else if (crManager.hasPingBoundarySetup(op->getName().getStringRef().str())) {
          crManager.addPongBoundaryOp(op, op->getNextNode()); // Start and end are the same op
          crManager.addPongBoundaryOp(op, op->getNextNode());
        } else {
          crManager.addPingBoundaryOp(op, op->getNextNode()); // Start and end are the same op
    ...
    auto maxIt = std::max_element(loopDepth.begin(), loopDepth.end(),
    ...
    while (currentOp) {
      ...
      // Stop at terminator
      if (!currentOp || currentOp == innermostForOp.getBody()->getTerminator()) {
        break;
      }
    ...
    if (crManager.hasPingPongBoundarySetup(expOp->getName().getStringRef().str())) {
      return;  // Already setup, skip
    } else if (crManager.hasPingBoundarySetup(expOp->getName().getStringRef().str())) {
      crManager.addPongBoundaryOp(expOp, expStartOp); // Start and end are the same op
      crManager.addPongBoundaryOp(expOp, expEndOp);
    } else {
      crManager.addPingBoundaryOp(expOp, expStartOp); // Start and end are the same op
  ...
  for (auto &pingEntry: opToPingBoundary) {
    ...
    builder.create<ttng::NamedBarrierWaitOp>(pingStart->getLoc(), pingBarrier, pingNumThreads);
    builder.setInsertionPoint(pingEnd);
    builder.create<ttng::NamedBarrierArriveOp>(pingEnd->getLoc(), pongBarrier, pingNumThreads);
    ...
    Value pingBarrier2 = builder2.create<arith::ConstantIntOp>(pongRegionLoc, *pingBarrierId, 32);
    Value pongBarrier2 = builder2.create<arith::ConstantIntOp>(pongRegionLoc, *pongBarrierId, 32);
    Value pingNumThreads2 = builder2.create<arith::ConstantIntOp>(pongRegionLoc, 256, 32);
    builder2.setInsertionPoint(pongStart);
    builder2.create<ttng::NamedBarrierWaitOp>(pongStart->getLoc(), pongBarrier2, pingNumThreads2);
