...
@triton.jit
def sr_convert_kernel(src, dst, BLOCK_SIZE: tl.constexpr):
    idxs = tl.program_id(0) * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    x = tl.load(src + idxs)
    y = x.to(dst.dtype.element_ty, fp_downcast_rounding='stne')
    tl.store(dst + idxs, y)


@triton.jit
def sr_convert_seeded_kernel(src, dst, seed: tl.constexpr, BLOCK_SIZE: tl.constexpr):
    idxs = tl.program_id(0) * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    x = tl.load(src + idxs)
    y = x.to(dst.dtype.element_ty, fp_downcast_rounding='stne', _rounding_seed=seed)
    tl.store(dst + idxs, y)
...
def _run_sr_conversion(src_tensor, dst_torch_dtype, device, seed=None, BLOCK_SIZE=1024):
    ...
    if seed is not None:
        sr_convert_seeded_kernel[grid](src_tensor, dst, seed, BLOCK_SIZE)
    else:
        sr_convert_kernel[grid](src_tensor, dst, BLOCK_SIZE)
    return dst
